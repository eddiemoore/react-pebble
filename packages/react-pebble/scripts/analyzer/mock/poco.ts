/**
 * poco.ts — PocoRenderer used by the Mock Renderer during Analyzer perturbation.
 *
 * Walks the pebble-dom tree and issues draw calls against a Poco-shaped
 * instance (MockPoco from ./render.ts in compile-time; never executed on-watch).
 * Notes on the Poco surface area we lower against:
 *  - No stateful color/font — every draw takes both as arguments. We cache
 *    name → PocoColor / PocoFont per renderer instance.
 *  - No native line, circle, or stroked rectangle. Outlines decompose into
 *    four thin fillRectangles; circles use midpoint algorithm; diagonal lines
 *    use Bresenham — both decomposed into fillRectangles.
 *  - Text alignment is manual — drawText draws at a point; we measure with
 *    getTextWidth and compute the origin for center/right alignment.
 *
...
import type { DOMElement, NodeProps } from '../../../src/pebble-dom.js';
