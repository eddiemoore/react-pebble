 ...
 *
 * Self-contained: owns its font/skin/style/texture tables, draw-call lowering,
 * and emission entry point. The Target Interface is the only public surface;
 * tests drive this Adapter through `piuTarget.emit(ir, ctx)`.
 *
...
const FONT_TO_PIU: Record<FontKey, string> = {
...
function fontToPiu(name: string | undefined): string {
  ...
  const mapped = FONT_TO_PIU[name as FontKey];
...
function emitIRNode(
  ...
  switch (el.type) {
    ...
    case 'rect': {
      ...
      // Piu's RoundRect supports only a single uniform radius — collapse
      // per-corner values to their max (or use uniform borderRadius).
    ...
    case 'image': {
      // Moddable's piu Texture only decodes PNG/JPEG. Fall back to a `.png`
      // sibling for APNG/PDC-sequence sources and warn.
      let src = el.src!;
    ...
    case 'canvas': {
      // onDraw callback is runtime-only — compiled output renders an empty
      // Port that user code can populate with a Behavior at runtime.
      const sizeProps = buildSizeProps(el.x, el.y, el.w, el.h, ir.platform.width, ir.platform.height);
      const nameProp = el.name ? `, name: "${el.name}"` : '';
...
function emitPiu(ir: CompilerIR, exampleName?: string): string {
  ...
  if (hasBehavior) {
    ...
    if (ir.hasTimeDeps) {
      ...
      // Bind once and store on `this` so onUndisplaying can remove the exact
      // same listener reference.
