 ...
 *
 * Self-contained: owns its font/color/alignment tables, element classification,
 * draw_proc lowering, TextLayer/Layer assembly, AppMessage wiring, and
 * configuration plumbing. The Target Interface is the only public surface.
 *
 * Honors `ir.appMessageSizes` when callers want to bound the AppMessage
 * buffer (env vars APPMSG_INBOX_SIZE / APPMSG_OUTBOX_SIZE in the CLI).
 *
...
const FONT_TO_C: Record<FontKey, string> = {
...
function fontToC(name: string | undefined): string {
  ...
  return FONT_TO_C[name as FontKey] ?? 'FONT_KEY_GOTHIC_18';
}
